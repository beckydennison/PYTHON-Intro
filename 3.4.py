...
def draw_chess_board(startx, endx, starty, endy):
    ...
    #starty = 0 # starting y position of the chess board

    box_size = (endx-startx)/8

    for i in range(0,8): # 8x8 chess board

        for j in range(0,8):

            draw_box(board, startx +j*box_size,starty+i*box_size,box_size,square_color)
...
draw_chess_board(-400,0,0,400)
draw_chess_board(100,500,0,400)
